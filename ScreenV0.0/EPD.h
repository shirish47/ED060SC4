...
#include "Hardware.h"
#include "var.h"// new defined typedefs for this file

GDISPControl_t GDISP;
...
static void inline vclock_quick()
{
    setpin_ckv(TRUE);
    eink_delay(1);
    setpin_ckv(FALSE);
 ...
 *       starting a new scan.
 */
static void inline vscan_start()
{
    setpin_gmode(TRUE);
    vclock_quick();
    setpin_spv(FALSE);
 ...
 */
static void inline vscan_write()
{
    setpin_ckv(TRUE);
    setpin_oe(TRUE);
    eink_delay(5);
 ...
 */
static void inline vscan_bulkwrite()
{
    setpin_ckv(TRUE);
    eink_delay(20);
    setpin_ckv(FALSE);
...
static void inline hscan_start()
{
...
static void inline hscan_write(const uint8_t *data, int count)
{
    while (count--)
    {
...
static void inline power_on()
{
 ...
 //   setpower_vdd(TRUE);
    ...
    delayMicroseconds(100);//gfxSleepMicroseconds(100);
    
    /* Then negative voltages and min. 1000 microsecond delay. */
    setpower_vneg(TRUE);
    delayMicroseconds(1000);//gfxSleepMicroseconds(1000);
...
static void inline power_off()
{
    ...
    delayMicroseconds(100);//gfxSleepMilliseconds(100);
...
static bool blocks_on_row(unsigned by)
{
    unsigned bx;
...
static void inline write_block_row(unsigned by)
{
    unsigned bx, dy, dx;
...
static void inline clear_block_map()
{
    unsigned bx, by;
...
static void inline flush_buffers()
{
    unsigned by, dy, i;
...
static void inline zero_block(block_t *block)
{
    unsigned dx, dy;
    for (dy = 0; dy < EINK_BLOCKHEIGHT; dy++)
    {
...
bool gdisp_lld_init(void)
{
    ...
    GDISP.Orientation =0;// GDISP_ROTATE_0;
...
void inline gdisp_lld_draw_pixel(coord_t x, coord_t y, color_t color)
{
    block_t *block;
    uint8_t byte;
...
void inline gdisp_lld_control(unsigned what, void  *value) {
...
static void inline subclear(color_t color)
{
    unsigned x, y;
    uint8_t byte;
...
void inline gdisp_lld_clear(color_t color)
{
    unsigned i;
    clear_block_map();
    ...
    {
        subclear(!color);
         delay(50);//   gfxSleepMilliseconds(50);
    }
    
    for (i = 0; i < EINK_CLEARCOUNT; i++)
    {
        subclear(color);
        delay(10);//gfxSleepMilliseconds(10);
    }
